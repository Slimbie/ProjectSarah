...
class SystemSkill(BaseSkill):
    ...
    wifi aan/uit, en een screenshot maken.
    """
    ...
    def _handle_volume(self, text: str) -> str:
        ...
        if "mute" in text or ("off" in text and "on" not in text):
            ok = self._run(["pactl", "set-sink-mute", "@DEFAULT_SINK@", "1"])
    ...
    def _handle_brightness(self, text: str) -> str:
        if "dim" in text or any(w in text for w in ["down", "decrease"]):
            ...
            return "Brightness decreased." if ok else "I couldn't change the brightness. Is brightnessctl installed?"
        if "brighten" in text or any(w in text for w in ["up", "increase"]):
            ok = self._run(["brightnessctl", "set", "10%+"])
            return "Brightness increased." if ok else "I couldn't change the brightness. Is brightnessctl installed?"
